...
def verify_parameters(digits, spacing_range, image_width):
	if not hasattr(digits, '__iter__'):
		raise TypeError('digits must be a list-like of ints')
	for digit in digits:
		if not isinstance(digit, int):
			raise TypeError('digits must be list-like of ints')
...
class DigitsImage(object):
	...
	def choose_random_digit_images(self):
		self.digit_images = MNISTDigits.get_rescaled_random_images(self.digits)
